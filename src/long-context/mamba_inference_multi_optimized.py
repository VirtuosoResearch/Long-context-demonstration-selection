"""
Optimized DLS (Distilled Layer-grouped SSM) implementation.

Key CUDA/GPU optimizations over the original:
  1. Precomputed A powers  — eliminates O(C) matmuls per chunk per scan call
  2. Batched group scan    — all G groups processed in parallel via a single einsum
  3. Fused B projection    — single batched einsum replaces G sequential nn.Linear calls
  4. Zero-copy cache expand — torch.expand replaces copy.deepcopy + batch_repeat_interleave
  5. torch.inference_mode  — lighter than torch.no_grad for eval paths
  6. Mixed-precision (AMP)  — optional torch.cuda.amp for training
  7. torch.compile support  — optional JIT compilation for the sidecar hot path
  8. Pre-allocated buffers  — .new_zeros avoids repeated device/dtype lookups
"""
...
# =====================================================================
# Per-group SSM compressor  (OPTIMIZED: precomputed A powers)
# =====================================================================

class LayerGroupSSM(nn.Module):
    """
    Single-group SSM with precomputed A powers.

    Optimization 1 — Precomputed A powers:
        The original code recomputes A^0, A^1, ..., A^{C-1} via C matrix
        multiplications *every* chunk *every* forward call.  With D=512 and
        C=64, that is 62 × 512² ≈ 16 M FLOPs wasted per chunk.  We now
        compute them once in __init__ and store as frozen buffers.

    This class is still instantiated per-group inside the sidecar so that
    checkpoint loading (groups.i.B.weight, groups.i.proj.*) is unchanged.
    The actual scan is done by the sidecar's batched path; this standalone
    scan serves as fallback / reference.
    """
    ...
    def __init__(self, hidden_size, ssm_dim, num_layers_in_group, num_kv_heads,
        ...
        # Discretised HiPPO-LegS A — FROZEN
        ...
        # ── OPT-1: Precompute A powers ──────────────────────────────
        # A_powers[i] = A^i for i = 0 … C  (inclusive)
        C = scan_chunk_size
        powers = [torch.eye(ssm_dim)]
        cur = torch.eye(ssm_dim)
        for _ in range(C):
            cur = cur @ a_disc
            powers.append(cur)
        powers_tensor = torch.stack(powers)              # [C+1, D, D]
        self.register_buffer("A_powers", powers_tensor)

        # full_scan_weights[t] = A^{C-1-t}  for t = 0 … C-1
        # i.e. reversed order: [A^{C-1}, A^{C-2}, …, A^0]
        self.register_buffer(
            "full_scan_weights",
            powers_tensor[:C].flip(0).contiguous()       # [C, D, D]
        )
        # ─────────────────────────────────────────────────────────────
    ...
    # Standalone scan — used only when the group is called directly.
    # In normal operation the sidecar's batched_scan is used instead.
    def scan(self, x, chunk_size=None):
        ...
        u_all = self.B(x)                               # [B, T, D]
        state = u_all.new_zeros(bsz, D)

        A_pw = self.A_powers.to(dtype=u_all.dtype)       # [C+1, D, D]
        fsw  = self.full_scan_weights.to(dtype=u_all.dtype)  # [C, D, D]
        ...
        for start in range(0, T, C):
            ...
            if c == 1:
                state = state @ A_pw[1].T + u_chunk[:, 0]
                continue

            # Pick pre-built weights (no matmuls needed)
            if c == C:
                weights = fsw                            # [C, D, D]
            else:
                weights = fsw[C - c:]                    # [c, D, D]  (last-chunk partial)

            total_contrib = torch.einsum('cdj,bcj->bd', weights, u_chunk)
            state = state @ A_pw[c].T + total_contrib
...
# =====================================================================
# Sidecar  (OPTIMIZED: batched B projection + batched scan)
# =====================================================================

class LayerGroupSSMSidecar(nn.Module):
    """
    Optimization 2 — Batched group processing:
        Instead of running G sequential scans, we:
        (a) fuse all B projections into a single einsum  [B,T,H] × [G,D,H] → [G,B,T,D]
        (b) run a single batched scan with state shape [G,B,D]
        This launches ~G× fewer CUDA kernels and keeps the GPU saturated.

    Optimization 3 — Shared precomputed A powers:
        All groups share the same frozen A, so we precompute A powers once at
        the sidecar level instead of storing G identical copies.
    """

    def __init__(self, config, ssm_dim=512, num_virtual_tokens=16, num_groups=4,
                 dt=1.0, scan_chunk_size=64):
        super().__init__()
        ...
        # Per-group modules — kept for checkpoint compatibility
        self.groups = nn.ModuleList([
            LayerGroupSSM(config.hidden_size, ssm_dim, e - s, self.num_kv_heads,
                          self.head_dim, num_virtual_tokens, dt, scan_chunk_size)
            for s, e in self.group_ranges
        ])

        # ── OPT-3: Shared A powers (avoid G identical copies) ───────
        # Reuse group 0's precomputed buffers; all groups have the same A.
        C = scan_chunk_size
        self.register_buffer("A_powers", self.groups[0].A_powers.clone())
        self.register_buffer("full_scan_weights", self.groups[0].full_scan_weights.clone())
        # ─────────────────────────────────────────────────────────────

        info = ", ".join(f"G{g}:[{s},{e})" for g, (s, e) in enumerate(self.group_ranges))
        print(f"[Sidecar] {num_groups} groups, ssm_dim={ssm_dim}, "
              f"vtokens={num_virtual_tokens}, chunk={scan_chunk_size} | {info}")

    # ── OPT-2: Batched scan across all G groups ─────────────────────
    def _batched_scan(self, u_all):
        """
        Process all G groups in parallel.

        Args:
            u_all: [G, B, T, D]  — B-projected embeddings per group
        Returns:
            state: [G, B, D]    — final SSM state per group
        """
        G, bsz, T, D = u_all.shape
        C = self.scan_chunk_size
        dtype = u_all.dtype

        state = u_all.new_zeros(G, bsz, D)
        A_pw = self.A_powers.to(dtype=dtype)             # [C+1, D, D]
        fsw  = self.full_scan_weights.to(dtype=dtype)    # [C, D, D]

        for start in range(0, T, C):
            end = min(start + C, T)
            c = end - start
            u_chunk = u_all[:, :, start:end, :]          # [G, B, c, D]

            if c == 1:
                # state: [G,B,D] @ [D,D] -> [G,B,D]  +  [G,B,D]
                state = state @ A_pw[1].T + u_chunk[:, :, 0]
                continue

            weights = fsw if c == C else fsw[C - c:]     # [c, D, D]

            # Single batched einsum: [c,D,D] × [G,B,c,D] → [G,B,D]
            total_contrib = torch.einsum('cdj,gbcj->gbd', weights, u_chunk)
            state = state @ A_pw[c].T + total_contrib
    ...
    def forward(self, embeddings):
        """
        Args:
            embeddings: [B, T, H]  — token embeddings of the demo prefix
        Returns:
            list of (K, V) tuples, one per transformer layer
        """
        bsz = embeddings.shape[0]

        # ── OPT-2a: Fused B projection via batched einsum ───────────
        # Stack B weights from all groups:  [G, D, H]  and  [G, D]
        B_w = torch.stack([g.B.weight for g in self.groups])   # [G, D, H]
        B_b = torch.stack([g.B.bias   for g in self.groups])   # [G, D]
        # Single fused matmul:  [B,T,H] × [G,D,H]^T → [G,B,T,D]
        u_all = torch.einsum('bth,gdh->gbtd', embeddings, B_w)
        u_all = u_all + B_b[:, None, None, :]
        # ─────────────────────────────────────────────────────────────

        # ── OPT-2b: Batched scan ────────────────────────────────────
        states = self._batched_scan(u_all)                     # [G, B, D]
        # ─────────────────────────────────────────────────────────────

        # Per-group MLP projection (different output sizes possible)
        all_kv = []
        for gi, group in enumerate(self.groups):
            nlayers = self.group_ranges[gi][1] - self.group_ranges[gi][0]
            flat = group.proj(states[gi])
            r = flat.view(bsz, nlayers, 2,
                          self.num_kv_heads, self.num_virtual_tokens, self.head_dim)
            for i in range(nlayers):
                all_kv.append((r[:, i, 0], r[:, i, 1]))
        return all_kv


# =====================================================================
# Cache utilities  (OPTIMIZED: avoid deepcopy)
# =====================================================================
...
def _fresh_cache_copy(cache):
    """
    OPT-4: Direct layer-wise clone instead of legacy roundtrip.
    Avoids Python-level deepcopy overhead while still producing an
    independent copy for use_cache=True forward passes.
    """
...
def _expand_cache_for_batch(cache, bsz):
    """
    OPT-5: Zero-copy cache expansion.
    Uses torch.expand (a view, no data copy) instead of
    copy.deepcopy + batch_repeat_interleave.
    Safe when the downstream forward uses use_cache=False.
    """
    if isinstance(cache, DynamicCache):
        expanded = DynamicCache()
        for i in range(len(cache.key_cache)):
            expanded.update(
                cache.key_cache[i].expand(bsz, -1, -1, -1),
                cache.value_cache[i].expand(bsz, -1, -1, -1),
                i,
            )
        return expanded
    # Fallback
    legacy = _cache_to_legacy(_ensure_cache_obj(cache))
    expanded = DynamicCache()
...
# =====================================================================
# Scorer  (OPTIMIZED: inference_mode + zero-copy expand)
# =====================================================================
...
class SSMHybridICLScorer:
    ...
    @torch.inference_mode()            # OPT-6: lighter than torch.no_grad
    ...
    def score_options_nll(self, first_logits, past_after_q, q_len, options):
        ...
        # ── OPT-5: zero-copy expand instead of deepcopy ─────────────
        bp = _expand_cache_for_batch(past_after_q, bsz)
        # ─────────────────────────────────────────────────────────────
...
# =====================================================================
# Training helpers  (OPTIMIZED: AMP support)
# =====================================================================
...
# =====================================================================
# Training loop  (OPTIMIZED: AMP, torch.compile, CUDA improvements)
# =====================================================================
...
def run_distillation_training(args):
    ...
    # ── Pre-tokenize demo texts to avoid repeated tokenization ───────
    demo_ids_list = []
    for dt_text in demo_texts:
        ids = tokenizer(dt_text, return_tensors="pt",
                        add_special_tokens=False)["input_ids"].to(device)
        if ids.numel() > 0:
            demo_ids_list.append(ids)
        else:
            demo_ids_list.append(None)
    print(f"Pre-tokenized {len(demo_ids_list)} demo texts.")
    # ─────────────────────────────────────────────────────────────────

    sidecar = LayerGroupSSMSidecar(
        model.config, ssm_dim=args.ssm_dim, num_virtual_tokens=args.num_virtual_tokens,
        num_groups=args.num_groups, scan_chunk_size=args.scan_chunk_size,
    ...
    # ── OPT-7: Optional torch.compile ────────────────────────────────
    if args.compile_sidecar and hasattr(torch, "compile"):
        print("[OPT] Compiling sidecar with torch.compile ...")
        sidecar = torch.compile(sidecar, mode="reduce-overhead")
    # ─────────────────────────────────────────────────────────────────
    ...
    # ── OPT-8: Mixed-precision scaler ────────────────────────────────
    use_amp = args.use_amp and device.startswith("cuda")
    amp_dtype = torch.float16 if not args.amp_bf16 else torch.bfloat16
    scaler = torch.cuda.amp.GradScaler(enabled=use_amp and amp_dtype == torch.float16)
    if use_amp:
        print(f"[OPT] AMP enabled with dtype={amp_dtype}")
    # ─────────────────────────────────────────────────────────────────
    ...
    for epoch in range(args.epochs):
        ...
        for di in tqdm(order, desc=f"epoch-{epoch+1}"):
            ...
            del teacher_kv, virtual_kv, sink_kv, loss, loss_kv, loss_logit
    ...
    if args.save_sidecar_path:
        ...
        save_sidecar = sidecar._orig_mod if hasattr(sidecar, "_orig_mod") else sidecar
        torch.save({"sidecar": save_sidecar.state_dict(), "args": vars(args),
                     "model_name": args.model_name}, args.save_sidecar_path)
        print(f"Saved: {args.save_sidecar_path}")

    save_sidecar = sidecar._orig_mod if hasattr(sidecar, "_orig_mod") else sidecar
    return {k: v.detach().cpu() for k, v in save_sidecar.state_dict().items()}


# =====================================================================
# Eval experiment  (OPTIMIZED: inference_mode, zero-copy expand)
# =====================================================================
...
def run_experiment(args, sidecar_state_dict=None):
    ...
    # ── OPT-7: Optional torch.compile for eval ──────────────────────
    if args.compile_sidecar and hasattr(torch, "compile"):
        print("[OPT] Compiling sidecar for eval ...")
        sidecar = torch.compile(sidecar, mode="reduce-overhead")
    # ─────────────────────────────────────────────────────────────────
    ...
    for dp in tqdm(eval_data, desc="eval"):
        ...
        # ─── SSM ───
        with torch.inference_mode():
            emb = model.get_input_embeddings()(demo_ids).to(dtype=next(
                (sidecar._orig_mod if hasattr(sidecar, "_orig_mod") else sidecar
                ).parameters()).dtype)
            ...
            if st > 0:
                sink_kv = model(input_ids=demo_ids[:, :st], use_cache=True).past_key_values
            ssm_demo_len = st + (sidecar._orig_mod if hasattr(sidecar, "_orig_mod")
                                 else sidecar).num_virtual_tokens

        scorer = SSMHybridICLScorer(model, tokenizer, device,
                                    sidecar._orig_mod if hasattr(sidecar, "_orig_mod") else sidecar,
...
# =====================================================================
# Loss comparison (standalone)  (minor: inference_mode)
# =====================================================================
...
def _load_sidecar(sidecar, args, device, state_dict=None):
    # Keys to skip: frozen buffers that are recomputed in __init__
    _skip_suffixes = ('.A', '.A_powers', '.full_scan_weights',
                      '.eigenvalues', '.V', '.V_inv')
...
if __name__ == "__main__":
    ...
    p.add_argument("--scan_chunk_size", type=int, default=64,
                   help="Chunk size for the chunk-parallel SSM scan.")
    ...
    p.add_argument("--use_amp", action="store_true",
                   help="Enable automatic mixed precision for sidecar training.")
    p.add_argument("--amp_bf16", action="store_true",
                   help="Use bfloat16 instead of float16 for AMP.")
    p.add_argument("--compile_sidecar", action="store_true",
                   help="Apply torch.compile to the sidecar module.")
    # ─────────────────────────────────────────────────────────────────
