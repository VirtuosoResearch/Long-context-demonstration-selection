...
class KVCachedICLScorer:
    ...
    def build_demo_cache(self, demo_text: str):
        ...
        out = self.model(input_ids=demo_ids, use_cache=True)
    ...
    @torch.no_grad()
    def score_options_nll(self, first_logits, past_after_question, question_len: int, options: List[str]) -> Dict[str, float]:
...
def run_experiment(args):
    ...
    kv_proxy += _attention_proxy_full(demo_len)  # demo prefill once

    kv_preds = []
    for dp in tqdm(eval_data, total=len(eval_data), desc="kv-cache"):
        q_text, _ = _normalize_text(dp, is_first=False, add_newlines=add_newlines)
        q_len = len(tokenizer(q_text, add_special_tokens=False)["input_ids"])
        first_logits, q_past, q_len_runtime = kv.prefill_question(q_text)
