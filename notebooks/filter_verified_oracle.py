...
def main():
    ...
    parser.add_argument("--out", type=str, default="./classified")
    ...
    for ex in ds:
        ...
        if instance_id:
            category_instances[cat].append(instance_id)

    out_path = os.path.join(args.out, "swe_bench_test_bm25.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(category_instances, f, ensure_ascii=False, indent=2)

    print("Done. JSON saved at", out_path)
...
#     ds = load_dataset("princeton-nlp/SWE-bench_bm25_13K", split="dev")
...
#     csv_path = os.path.join(args.out, "swebench_verified_classification.csv")
#     df.to_csv(csv_path, index=False)

#     summary_rows = [{"category": c, "count": n} for c, n in cat_counter.most_common()]
#     df_sum = pd.DataFrame(summary_rows)
#     sum_path = os.path.join(args.out, "swebench_verified_classification_summary.csv")
#     df_sum.to_csv(sum_path, index=False)

#     if args.jsonl:
#         jsonl_path = os.path.join(args.out, "swebench_verified_classification.jsonl")
